...
anywhere in `R/` and not regenerated by `data-raw/build_data.R`. The
name "toursdata" appears in `vignettes/LinkTracks.Rmd` and as the id of
a
[`DT::dataTableOutput`](https://rdrr.io/pkg/DT/man/dataTableOutput.html)
in `app.R`, but both rebuild a local variable fresh from
`othervariables` (`toursdata <- othervariables %>% ...`) rather than
consuming this lazy-loaded object.
