...
Frozen-legacy. No [`save()`](https://rdrr.io/r/base/save.html) call
anywhere in `R/` and not regenerated by `data-raw/build_data.R`.
`vignettes/CombinationLock.Rmd` recomputes a same-named local variable
from `mydf3` (`transitions <- mydf3 %>% ...`, line 98) rather than
consuming this object - do not confuse that vignette's extensive use of
the name with this object being live. Distinct from
`transitions_data_da`, which is separately produced by `Repeatr_1` and
actively used.
