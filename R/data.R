...
#' \item{sound_quality}{Sound quality rating: Excellent, Very Good, Good, or Poor.}
#' }
#' @section Provenance: Derived, generated once (via \code{\link{sweepstack}}/\code{\link{stacks}}) and not regenerated by any current `R/` pipeline stage or by `data-raw/build_data.R` - do not assume it reflects the latest `duration_data_da`. Actively consumed directly by `inst/shiny/Fugazetteer/app.R` for the "stock" pages, so despite being un-regenerated it is not orphaned; confirm via `R/`, `vignettes/`, and `app.R` before ever tagging it (or anything else) `Frozen-legacy`.
#' @examples
...
#' }
#' @section Provenance: Frozen-legacy. No `save()` call anywhere in `R/` and not regenerated by `data-raw/build_data.R`. The name "venuesdata" does appear in `vignettes/LinkTracks.Rmd`, but only as a local variable rebuilt fresh from `othervariables` (`venuesdata <- othervariables %>% ...`) - that shadows rather than consumes this lazy-loaded object, so it should still be treated as orphaned unless a direct reference turns up elsewhere.
#' @examples
#' venuesdata
"venuesdata"
...
#' \item{seconds}{duration in seconds}
#' }
#' @section Provenance: Derived-cleaned. Produced by \code{\link{Repeatr_1}} from `fls_tags`.
#' @examples
#' fls_tags_show
"fls_tags_show"
