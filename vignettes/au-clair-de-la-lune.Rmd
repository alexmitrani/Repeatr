...
tbo_count <- fugazi_song_counts$count[fugazi_song_counts$title == "two beats off"]
tbo_last <- Repeatr1 %>% filter(title == "two beats off") %>% filter(date == max(date))
tbo_last_show <- shows_data %>% filter(gid == tbo_last$gid[1])
tbo_to_repeater <- transitions_data_da %>% filter(title1 == "two beats off", title2 == "repeater") %>% nrow()
```
...
Two Beats Off was launched as an instrumental at the 40 Watt in Athens, Georgia on 3 May 1989. The first rendition with vocals was at the 9:30 Club in Washington DC on 19 July 1989. The song got it's official release on Repeater which was released around the 1 March 1990 [@fugazi1990e]. It was one of the later songs to debut that would be released on Repeater.

```{r echo=FALSE}
releases_data_input %>%
  filter(release_title=="repeater") %>%
  select(title, date) %>%
  rename(debut = date) %>%
  arrange(debut)
...
Fugazi played Two Beats Off at least `r tbo_count` times with the last recorded version being on the `r as.integer(format(tbo_last$date[1], "%d"))` `r format(tbo_last$date[1], "%B %Y")` in `r tbo_last_show$city`, `r tbo_last_show$subdivision`, `r tbo_last_show$country`. Two Beats Off was followed by the song Repeater `r tbo_to_repeater` times which makes this by far the most common transition featuring the song.
...
> I put a kiss to your wrist but now
> I've got my head in the shape of a fist"
[@fugazi1990g]
