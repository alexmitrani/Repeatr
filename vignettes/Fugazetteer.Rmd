...
period_weeks_total <- round(as.numeric(diff(range(shows_data$date))) / 7)

glueman_from_shows <- sum(transitions_data_da$title1 == "glueman")
glueman_to_shows <- sum(transitions_data_da$title2 == "glueman")

swedish_2000_tour <- shows_data %>%
  filter(country == "Sweden", tour == "2000 Fall Northern European Tour")

swedish_sets <- sets(mydf = duration_data_da, shows = swedish_2000_tour$gid)
swedish_songs <- swedish_sets[[1]]
swedish_summary <- swedish_sets[[2]]

n_shows_with_setlists <- length(unique(gid_initial_gid_sound_quality$gid_initial))

stack_sizes <- gid_initial_gid_sound_quality %>%
...
[Fugazetteer](https://alexmitrani-fugazetteer.share.connect.posit.cloud/) makes it easy to get some insights and answer questions about the Fugazi Live Series. It also provides several new ways of finding shows to listen to.
...
-   flow (10 pages) - includes top-level menus for 'year' and 'tour' which will filter the contents of all the pages in this section.
...
The data for songs is limited to songs that were played live at least twice in the Fugazi Live Series. That is `r nrow(songidlookup)` songs which feature in the Fugazi discography. This limit does not apply to the 'details' and 'search' pages, which additionally include one-off and otherwise unreleased performances - see those pages below.
...
-   'home' will move the timeline slider back to the beginning and set the period to `r period_weeks_total` weeks again, to select the entire series. It is recommended to use the home button before changing the selections in the selection boxes above the map.

Pressing the 'home' button reveals additional controls:

-   period_weeks indicates the duration of the selected period in weeks. It is initially set to `r period_weeks_total` weeks which covers the full duration of the Fugazi Live Series.
...
The summary table tells us that `r nrow(swedish_songs)` unique songs were played: `r swedish_summary$songs[swedish_summary$shows==1]` songs were played only once, and `r swedish_summary$songs[swedish_summary$shows==max(swedish_summary$shows)]` songs were played at all `r nrow(swedish_2000_tour)` shows, with the remaining `r sum(swedish_summary$songs) - swedish_summary$songs[swedish_summary$shows==1] - swedish_summary$songs[swedish_summary$shows==max(swedish_summary$shows)]` songs played in some but not all of the shows. The `r nrow(swedish_2000_tour)` shows have `r sum(swedish_songs$shows)` songs in total, including the repeated performances.

The Details table shows the complete list of songs, and for each song which show or shows it was performed at. The table is sorted in descending order of the number of shows each song was performed at, so in this case the top part of the table lists the `r swedish_summary$songs[swedish_summary$shows==max(swedish_summary$shows)]` songs that were performed at all `r nrow(swedish_2000_tour)` shows.
...
Here you can choose any show (out of the `r n_shows_with_setlists` shows with set lists available to date) and then a stack that contains the selected show. The stacks are named after the initial show that was chosen when the stack was created. Many shows appear in only one stack but some shows appear in many stacks. Once you have chosen a show and a stack, you will get a short sequence of `r min(stack_sizes$n)`-`r max(stack_sizes$n)` shows including the selected shows and covering all `r nrow(songidlookup)` songs that Fugazi played live at least twice.
...
Here you can choose any available show and see a lot of information about it in one place. 
The first section includes the venue and date with a link to the corresponding page of the [Fugazi Live Series](https://www.dischord.com/fugazi_live_series) site, its distance from "home" (Washington, DC, where the first-ever Fugazi show took place), where it fits in its tour, how many times Fugazi had previously played in that country, state/province, city and venue. The previous and following show are also named, each with its own distance from this one when the two were part of the same touring trip (as opposed to two separate home-based outings) - this is worked out from geography and dates, not from the show's named tour, so it can occasionally differ from what the tour name alone would suggest.

If a recording of the show exists, the information includes how many songs were played, the length of the recording (and how much of that is music), who recorded it on what medium, its sound quality rating, and a summary of how many songs come from each release. 

There may or may not be a list of notes indicating anything unusual or noteworthy about the show or setlist. The notes, if there are any, will be limited to interesting facts that can be detected from the available data. For other interesting information about the show please refer to the corresponding page on the Fugazi Live Series site - there is a link to it below the title on the Recap page. 

An interactive map shows the location of the show - you can move the map and change the zoom level if you wish.

If a recording exists, a detailed tracklist is shown, listing every track on the recording - songs and non-song content alike (interludes, intros, outros, encores and other one-offs) - with each track's duration; the song's average recorded duration across all Fugazi shows; its position in the set and its average position across all recorded renditions; which numbered rendition of the song this was and its total number of recorded renditions; if it followed directly on from another song, which numbered occurrence of that song-to-song transition this was and its total number of occurrences across all shows; and the release date of the album or EP the song comes from.

The whole page can be downloaded as a PDF document using the download button at the top of the page to the right of the 'show' selection box.
...
Unlike the rest of the 'stock' section, the song selector here isn't limited to the `r nrow(songidlookup)` songs from the official Fugazi discography - it also includes one-off and otherwise unreleased performances not on any release, such as 'heart on my chest' or 'lock dug', so that as many of the original recorded performances as possible - including the occasional untitled one - stay browsable even when only a single instance was ever recorded.
...
Like 'details' above, the song selector here also includes one-off and otherwise unreleased performances not on any official release - so searching for something like 'lock dug' still surfaces the single show where it was played, rather than being limited to the `r nrow(songidlookup)` songs from the official Fugazi discography.
