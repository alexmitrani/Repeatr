...
## Did Fugazi randomly pick songs from their entire available repertoire for each show?

If songs were picked at random from all available songs, the play count for each song should build up in a similar way over time.  This can be checked by plotting the cumulative play counts for a selection of songs.  First, let's get the data into a suitable format.

```{r}

mydf <- Repeatr1 %>% select(date, song)
mydf <- mydf %>% group_by(date, song) %>% slice(1) %>% ungroup()
mydf <- mutate(mydf, counter=1)
mydf_wide <- mydf %>% pivot_wider(names_from = song, values_from = counter, values_fill = 0)
head(mydf_wide)

```

Next, let's transform the variable for each song into a cumulative count of the number of times the song was played, and rename the variables so the variable names work as they should.  

```{r}

for(colindex in 2:93) {
  
  mydf_wide[,colindex] <- cumsum(mydf_wide[,colindex])     
  
}

names(mydf_wide) <- make.names(names(mydf_wide))

head(mydf_wide)

```

Now, we can reformat the data again to make it a long list of song counts that show how the number of times each song was played built up over time.  
...
Now we are in a position to graph the data so see how the number of times each song was played evolved over time.  

This is the list of songs we can choose from:
...
Returning to the initial question of whether Fugazi picked songs randomly or not, it seems that there was some randomness but they also had preferences. They sometimes stopped playing certain songs for prolonged periods of time. For instance, there were lengthy periods where KYEO and Furniture were not being played at all, and then they were brought back and played again. Other songs such as Polish were dropped and did not return. Fugazi picked their songs with the randomness and preferences of a band, not the randomness of a machine. They tried [a different system for picking songs](https://www.dischord.com/fugazi_live_series/waukesha-wi-usa-61590) once but it only lasted about one song!    
